...
VARIABLES = ["intra_oxy", "intra_glu", "intra_lac", "intra_energy"]
# Define the time format to be plotted (minutes should be "min" and hours should be "h")
TIME_FORMAT = "h"


def get_simulation_time_interval(output_path: Path) -> float:
    """Returns the current time at the first simulation time point (excluding time point 0)."""
    tree = ElementTree.parse(output_path / "output00000001.xml")
...
def get_variables_idx(variables: List[str], output_path: Path) -> Dict[str, int]:
    ...
    return {
        label.text: int(label.attrib["index"])
...
def extract_data(
    frame: int, variables_map: Dict[str, int], output_path: Path, time_interval: float, time_format: str = "min"
) -> pd.DataFrame:
...
if __name__ == "__main__":
    ...
    frames = processing.get_cell_file_num(output_path=OUTPUT_PATH, version="1.10.2")
    ...
    for i, ax in enumerate(axes.ravel()):
        ...
        ax.set_xlabel(f"Time [{TIME_FORMAT}]", labelpad=10)
