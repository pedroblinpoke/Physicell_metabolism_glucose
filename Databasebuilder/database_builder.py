...
def update_SBML(new_k_gly: float, new_k_aer: float, new_k_ana: float, new_k_ene: float, new_k_prolif: float, new_death_thresh: float, new_prolif_thresh: float, old_params):
    #This function update the SBML demo.xml. The imputs of this function should be the new values of the parameters.

    #Path of the file to change
    xml_file_path = 'C:/Users/Usuario/Documents/GitHub/PhysiCell_base_velocity/config/demo.xml'
    #Path to write the changed file
...
def calculate_area_ConvexHull(timestep, folder_name):
    #Calculate the area at a particular timestep
...
def count_ones_in_list(lst):
    #This funtion count the number of ones in a list
    count = 0
    for item in lst:
        if item == 1:
            count += 1
...
def count_two_in_list(lst):
    # This funtion count the number of twos in a list
    count = 0
    for item in lst:
        if item == 2:
            count += 1
...
def count_the_clusters_and_cells(timestep, folder_name):
    #This function counts the number of cells and spheroids
...
def count_the_clusters(timestep, folder_name):
    #This function counts the number of clusters of cells at a particular timestep
    cells_data = get_cell_positions(timestep, folder_name)
...
def count_ind_cells(timestep, folder_name):
    # This function counts the number of individual cells at a particular timestep
...
def get_death_cell_percentage(timestep, folder_name):
    # This function get the death cell percentage at a particular timestep
...
def calculate_all_areas(total_time, folder_name):
    #Function that measure the measurements to perform on the spheroids and calculate their mean and error.
    timestep = 0
    #save_every=60
    day_0=0
    day_1=1
    ...
    day_6=6
    day_7=7
    #Define the days to save the data
    days_to_save=[day_0, day_1, day_2, day_3, day_4, day_5, day_6, day_7]
    #days_to_save = [day_0, day_1, day_2]
    ...
    while timestep < total_time+1:
        if timestep in days_to_save:
            #Calulate the area for this particular timestep
            spheroids_areas =  calculate_area_ConvexHull(timestep, folder_name)
            spheroids_areas = [value for value in spheroids_areas.values()]
            spheroids_areas_array= np.array(spheroids_areas)
            if spheroids_areas_array.size == 0:
                spheroids_areas_array = np.array([0])
            #print(f"The areas are:{spheroids_areas_array}")
            #Take the spheroids bigger than one or two cells
            ...
            # Calculate the mean area excluding the spheroids of one or two cells
            if spheroids_areas_no_1_2_array.size == 0:
                comp_df.loc[timestep, 'Size_not_1_or_2'] = 0
            if spheroids_areas_no_1_2_array.size == 1:
                comp_df.loc[timestep, 'Size_not_1_or_2'] = spheroids_areas_no_1_2_array
            ...
            #Mean number of free cells
            comp_df.loc[timestep, 'Num_of_1'] = count_ones_in_list(spheroids_areas)

            #Mean number of clusters with just two cells
            comp_df.loc[timestep, 'Num_of_2'] = count_two_in_list(spheroids_areas)

            #Mean number of clusters and free cells
            comp_df.loc[timestep, 'Nums_of_spheroids_and_cells'] = count_the_clusters_and_cells(timestep, folder_name)
            #Mean number of spheroids
            comp_df.loc[timestep, 'Nums_of_spheroids'] = count_the_clusters(timestep, folder_name)
            #Mean percentage of death cells
...
def divide_dataframes_by_cluster(dataframe):
    #This function divide a dataframe depending on the clusters
...
#Define the path of the PhysiCell carpet
new_directory = Path(f'C:/Users/Usuario/Documents/GitHub/PhysiCell_base_velocity_aux')
os.chdir(new_directory)
#Define the number of parameters
number_of_parameters=7
sampler = qmc.Sobol(d=number_of_parameters, scramble=False)
#Define the number of simulations of our database. Number of simulations= 2^m
...
#Define the limits we want to establish for the dataset
k_glu_min=1e-15
k_glu_max=1e-11
...
k_prolif_max=0.00077
k_prolif_min=0.00045
...
k_glu = (k_glu_max - k_glu_min) * k_glu + k_glu_min
k_aer = (k_aer_max - k_aer_min) * k_aer + k_aer_min
k_ana = (k_ana_max - k_ana_min) * k_ana + k_ana_min
k_ene = (k_ene_max - k_ene_min) * k_ene + k_ene_min
k_prolif = (k_prolif_max - k_prolif_min) * k_prolif + k_prolif_min
atp_sup = (atp_sup_max - atp_sup_min) * atp_sup + atp_sup_min
atp_inf = (atp_inf_max - atp_inf_min) * atp_inf + atp_inf_min
...
##### The parameters on demo.xml has to be set to 0.0 before the simulation manually
...
#Initialice the final dataframe to be writen as an output
...
#Create the files to be write the outputs
i=0
output_files = {
...
for i in range(int(num_of_params_config)):
    ...
    #Print this particular number of parameters
    ...
    #Number of simulations with this particular parameter combination
    num_simulations = 1  # Number of simulations to process
    #Clean previous PhysiCell files
    os.system("make data-cleanup")
    #Make the compilation of PhysiCell files
    os.system("make")
    ...
    #Write the dataframes into txt files
